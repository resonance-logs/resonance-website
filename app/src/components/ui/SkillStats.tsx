...
export default function SkillStats({ encounterId, playerId, durationSec, showTitle = false, classId, classSpec, playerName }: Props) {
  const enabled = !!encounterId && !!playerId;
  const [activeTab, setActiveTab] = useState<'damage' | 'heal'>('damage');

  // Determine player type
  const playerType = classId && classSpec ? getType(classId, classSpec) : "damage";
  const showDamageOnly = playerType === "tank" || playerType === "damage";
  const showHealOnly = playerType === "healer";
  const showBoth = playerType === "damagehealer";
  ...
  return (
    <div>
      ...
      <div className="w-full space-y-6">
        {/* Damage Skill Stats */}
        {(showDamageOnly || (showBoth && activeTab === 'damage')) && (
        <div className="rounded-lg border border-gray-800 bg-gray-900/40 overflow-hidden w-full">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-800/30">
              ...
              <tbody>
                {damageSkillStats.length === 0 ? (
                  ...
                  damageSkillStats.map((skill) => {
                    ...
                    return (
                      <tr key={skill.id} className="border-b border-gray-800/50 hover:bg-gray-800/40 transition-colors">
                        <td className="px-6 py-3 text-gray-300">{skillName}</td>
        ...
        {/* Heal Skill Stats */}
        {(showHealOnly || (showBoth && activeTab === 'heal')) && (
        <div className="rounded-lg border border-gray-800 bg-gray-900/40 overflow-hidden w-full">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-800/30">
