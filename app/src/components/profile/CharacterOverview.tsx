...
interface CharacterOverviewProps {
  ...
  isLoading?: boolean;
  onShare?: (playerId: number) => void | Promise<void>;
...
export function CharacterOverview({
  ...
  const isProfileLoading = isLoading && playerData.length === 0;

  const handleShareClick = async () => {
    if (!onShare || !player?.playerId) return;
    try {
      await onShare(player.playerId);
      ...
      console.error('Failed to share player', err);
    }
  };
                ...
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-sm font-semibold transition-colors ${
                    shareState === 'copied'
                      ...
                      : 'border-purple-500/40 bg-purple-500/10 text-purple-100 hover:border-purple-300'
                  }`}
                >
                  {shareState === 'copied' ? 'Link Copied!' : 'Share Player'}
                </button>
              )}
              {playerData.length > 1 && (
