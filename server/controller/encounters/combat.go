...
// GetCombatEvents retrieves combat events for a specific encounter
func GetCombatEvents(c *gin.Context) {
	db := c.MustGet("db").(*gorm.DB)
	encounterID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid encounter ID"})
	...
	offset, _ := strconv.Atoi(c.DefaultQuery("offset", "0"))
	eventType := c.Query("type") // damage, heal, death
	attemptIndex, _ := strconv.Atoi(c.DefaultQuery("attemptIndex", "1"))

	// Prepare response
	response := gin.H{
		"damageEvents": []models.DamageEvent{},
		"healEvents":   []models.HealEvent{},
		"deathEvents":  []models.DeathEvent{},
	}

	// Get damage events if requested
	if eventType == "" || eventType == "damage" {
		...
		result := db.Where("encounter_id = ? AND attempt_index = ?", encounterID, attemptIndex).
			Order("timestamp_ms ASC").Limit(limit).Offset(offset).Find(&damageEvents)

		if result.Error != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": result.Error.Error()})
			return
		}
		response["damageEvents"] = damageEvents
	}

	// Get heal events if requested
	if eventType == "" || eventType == "heal" {
		var healEvents []models.HealEvent
		result := db.Where("encounter_id = ? AND attempt_index = ?", encounterID, attemptIndex).
			Order("timestamp_ms ASC").Limit(limit).Offset(offset).Find(&healEvents)

		if result.Error != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": result.Error.Error()})
			return
		}
		response["healEvents"] = healEvents
	}

	// Get death events if requested
	if eventType == "" || eventType == "death" {
		...
		response["deathEvents"] = deathEvents
	}
...
// GetDamageEvents retrieves only damage events for a specific encounter
func GetDamageEvents(c *gin.Context) {
	db := c.MustGet("db").(*gorm.DB)
	encounterID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid encounter ID"})
	...
	// Get total count
	var total int64
	db.Model(&models.DamageEvent{}).Where("encounter_id = ? AND attempt_index = ?", encounterID, attemptIndex).Count(&total)
...
// GetHealingEvents retrieves only healing events for a specific encounter
func GetHealingEvents(c *gin.Context) {
	db := c.MustGet("db").(*gorm.DB)
	encounterID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid encounter ID"})
	...
	// Get total count
	var total int64
	db.Model(&models.HealEvent{}).Where("encounter_id = ? AND attempt_index = ?", encounterID, attemptIndex).Count(&total)
