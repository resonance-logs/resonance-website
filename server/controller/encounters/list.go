...
func GetEncounters(c *gin.Context) {
	...
	for _, b := range bosses {
		bossesBy[b.EncounterID] = append(bossesBy[b.EncounterID], b)
	}
	statsBy := map[int64][]models.ActorEncounterStat{}
	for _, s := range stats {
		statsBy[s.EncounterID] = append(statsBy[s.EncounterID], s)
	}
	...
	for _, e := range encs {
		...
		teamDps := 0.0
		if durationSec > 0 {
			teamDps = float64(e.TotalDmg) / durationSec
		}
		st := statsBy[e.ID]
		...
		for _, p := range st {
			...
			if p.AbilityScore != nil {
				sumAS += *p.AbilityScore
				cntAS++
			}
		}
		var avgAS *int64
		...
		if err := db.Select("discord_username").Where("id = ?", e.UserID).First(&user).Error; err == nil && user.DiscordUsername != "" {
			team = user.DiscordUsername + "'s Team"
		}
