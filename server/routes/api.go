package routes

import (
	"server/routes/groups"

	"github.com/gin-gonic/gin"
)

func RegisterAPIRoutes(router *gin.Engine) {
	// Attach auth middleware globally for /api/v1 to populate current user when session exists.
	rg := router.Group("/api/v1")

	groups.RegisterAuthRoutes(rg)
	groups.RegisterApiKeyRoutes(rg)
	groups.RegisterCombatRoutes(rg)
	groups.RegisterMyEncounterRoutes(rg) // Private encounter routes
	groups.RegisterPlayerRoutes(rg)
	groups.RegisterUploadRoutes(rg)
	groups.RegisterModuleRoutes(rg)
	groups.RegisterStatisticsRoutes(rg)
	groups.RegisterSettingsRoutes(rg)
	groups.RegisterCustomizationRoutes(rg)
	groups.RegisterKofiRoutes(rg)
	groups.RegisterEntityRoutes(rg)
	groups.RegisterTrackingRoutes(rg)
	groups.RegisterMarketRoutes(rg)

}
